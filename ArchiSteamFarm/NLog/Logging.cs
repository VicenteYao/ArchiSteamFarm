...
namespace ArchiSteamFarm.NLog {
	internal static class Logging {
		...
		private static NamedPipeClientStream NamedPipeClientStream;
		...
		internal static async Task<string> GetUserInput(ASF.EUserInputType userInputType, string botName = SharedInfo.ASF) {
			...
			if (NamedPipeClientStream==null) {
				NamedPipeClientStream = new NamedPipeClientStream(botName);
				...
				try {
					...
					switch (userInputType) {
						...
						case ASF.EUserInputType.SteamGuard:
							...
							{
								string steamGuard = "SteamGuard";
								SendMessage(steamGuard);
							}
		...
		private static void SendMessage(string steamGuard) {
			var bytes = Encoding.UTF8.GetBytes(steamGuard);
		...
		private static string ReadMessage() {
			...
			do {
				...
				int numBytes = NamedPipeClientStream.Read(bytes, 0, bytes.Length);
